...
> Status of every ticket lives in `TRACKER.md`; this file holds the layer definitions and ticket specs.
...
- **Walks away mid-game -> the visible flush.** After a verdict, if nobody plays for 12 s, a countdown appears on screen ("Still there? Resetting in N..."), which runs 6 s - 18 s in total. (Shortened from 30 s on the owner's call, 2026-09-22: a queue waiting on a kiosk still showing the last visitor's session costs more than challenging someone who is still there, which is one tap to dismiss.) At zero the screen flushes with a short animation back to the attract state, ready for the next person. Any tap during the countdown cancels it. The server's own 60 s idle reset stays as the safety net if the browser is gone.
...
| C5 | **Tasks and gifts on the web.** Every reward shown comes from the server (`claim_task` / `free_refill` return the reward and the new balance); the tasks screen reflects claimed/repeatable state from the server, not localStorage. | server + client | see TRACKER.md|
| C6 | **Win and lose feedback on the web.** Verdict pane driven only by `round_settled`; streak and multiplier shown from the frame; the 2 s "settling" state; the "quiet market" flat explained on screen. | client | see TRACKER.md|
| C7 | **Broke on the web.** Clear path from 0 coins to refill and tasks; never a dead end. | client | see TRACKER.md|
| C8 | **Kiosk with no prize codes left.** The pool empty is a product state, not an edge case: `open_kiosk_round` refuses with `coupons_exhausted` when no code is available; `kiosk_session` carries `codes_left`; the kiosk shows a full-screen modal over the play screen instead of Play, and returns to normal by itself when staff load codes (the server pushes `kiosk_session` to every kiosk when the count crosses zero either way). Copy: title "All the prizes are gone", body "Every $100 code for today has been won. Tell someone at the xChief booth you'd like to play - they can load more prizes in a minute.", small line "This screen updates by itself." | server + client | see TRACKER.md |
| D1 | **Monitoring for the campaign.** Dozzle in compose (live container logs in a browser, behind Cloudflare Access or Caddy basic auth), a `/status` page (feed sources, rounds/min, open sockets, coupons left), and one alert (feed silent > 60 s, or server restart) to a phone. | ops | see TRACKER.md|
...
| S17 | MT5 price feed: `server/feed-mt5.js` is built and merged at priority 0 behind `METAAPI_TOKEN` + `METAAPI_ACCOUNT_ID` + `METAAPI_SYMBOL` (S17-prep done 2026-09-16). Remaining: install `metaapi.cloud-sdk` on the box, set the three values, run `demo/feed-compare.mjs` for 10 minutes against the bar in `mt5-feed.md`. Blocked on the investor login and MetaApi token. |
