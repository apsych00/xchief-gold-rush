...
**2026-09-18 testing round (owner walked the preview, fixes fanned out and merged):**

- **Instagram (K3) fixed end to end.** Root causes: `.env.box` `BOXAPI_BASE` was wrong (`/instagram-api` vs the correct `/api/instagram/`, corrected in place); the adapter did not unwrap BoxAPI's `response.body` envelope (`server/instagram.js` `pickBody`); the `instagram://` deep link errored on desktop (now opens the web profile URL, works on both); the follow modal had no escape (added a Cancel/close); and a client check-flow race let a benign `rate_limited`/early `not_following` stomp a confirmed follow (single-flight + success latch in `src/Tasks.jsx`). Verified: real BoxAPI confirms a real follower, fresh-DB E2E green. Live once `BOXAPI_TOKEN` is set (it is on the preview).
- **Quiet-market simulation ported to the server feed.** The flat-chart bug: `server/feed.js` only republished the raw upstream, and off-hours gold sends frequent same-price ticks so `quiet` (keyed on tick arrival) never fired. Now quiet is movement-based (price unchanged > 3 s) and the feed synthesizes a bounded mean-reverting walk (+-0.35, 3-decimal) anchored to the last real price, resuming the real series smoothly. Server-decides intact (rounds read the server's own published price; client cannot influence it). Client honors the server `quiet` flag. 31/31 feed tests.
- **Missions:** Coins -> Missions with one YouTube row cycling 3 videos (30 s watch, server-released, 3-min unlock, "x of 3", white overlay note + Skip); Register-at-xChief opens `my.xchief.com/registration?utm_source=goldrush` with a 1 h localStorage-persisted timer; brand social icons fixed; the leftover solo hosted-video row removed; Google/Forex-Peace-Army reviews removed.
- **YouTube embed hardened** (nocookie host, `origin`, user-gesture playback, "open on YouTube" fallback) against the "confirm you're not a robot" interstitial (which is IP-reputation based and cannot be defeated client-side).
- **Kiosk:** open `/kiosk` route is the booth mechanism (self-provisions, no secret); self-heals a stale/revoked secret (clears + re-provisions) so a DB reset or redeploy never bricks a booth device; iPad Air layout (kiosk + web) filling to a thin edge, shorter console body, countdown fits; clean English intro; "Play on web" QR on the attract screen; Up/Down lock like web (Play Again to continue).
- **Share reworked to an image** (rendered badge/pass with the real header logo + native share sheet or desktop download + join link); the public `/s/<token>` page, token column, and copy-link were removed (killed the "something went wrong" mint bug).
- **Claim page:** the $100 code is selectable with a Copy button; email input sized to match the app. The code email is transactional plain text (no unsubscribe in our code; a designed template is a later task).
- **Polish:** English-only sweep across all screens; logo links home; animated glow background; ads clickable to xchief.com with the goldrush UTM and no white flash; leaderboard email prompts hide once verified; tournament prize set to $3,000 broker bonus.
- **Ops/deploy:** shared-box runbook (`docs/deploy-shared-box.md`, Cloudflare Tunnel origin that avoids the box's occupied ports) + `docker-compose.box.yml`; private deploy repo `nightmareinc/xchief-gold-rush`; removed the dead Supabase edge functions/docs, the unused `@supabase/*` packages, and 70 tracked E2E screenshot artifacts (now git-ignored).
- **Testing gate during this round:** surgical per-feature specs plus lint/build, run centrally by the orchestrator one at a time (no full-suite, no worker-run tests) to avoid CPU contention. Workers ran in isolated worktrees; the orchestrator reviewed and merged.

Remaining after this round: the play-button restyle (owner), the box deployment run (owner - needs SSH/Cloudflare tunnel token/secrets from the runbook), a designed claim-code email template (later), the Elastic Email account-level unsubscribe toggle (owner dashboard), and two Instagram field edges to watch (200+ following pages out; BoxAPI response shapes handled defensively).
...
**2026-09-18 early: the machine went down with four workers in flight; the exact resume state is in `docs/HANDOFF.md`. Read that first; the block below is the older operating state and still describes the dev-client stack.**
...
| U4 | Share: modal with banner preview, copy link and social shortcuts; public `/s/<token>` page with CTAs to play | change | ⏸️ stalled by the 2026-09-17 21:35 network drop, machine down; uncommitted work in its worktree; resume per docs/HANDOFF.md |
| S19 | MT5 route security audit and hardening (A1) | | 📋 ready to ticket |
| K1 | Open kiosk route `/kiosk`, self-provisioning with a locally persisted identity (accepted leak, decommission after the expo) | change | ⏸️ stalled by the 2026-09-17 21:35 network drop, machine down; uncommitted work in its worktree; resume per docs/HANDOFF.md |
...
| K4 | Coins tab becomes Missions: YouTube missions with a non-seekable watch-progress player; redirect rewards release the moment the timer ends | change | ⏸️ stalled by the 2026-09-17 21:35 network drop, machine down; uncommitted work in its worktree; resume per docs/HANDOFF.md |
...
| D5 | Screenshots of every screen (separate PNGs + one high-quality sprite) for a design agent, git-ignored under `design/screens/` (owner 2026-09-17 late) | one-off | ⏸️ stalled by the 2026-09-17 21:35 network drop, machine down; uncommitted work in its worktree; resume per docs/HANDOFF.md |
