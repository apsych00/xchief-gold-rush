...
| B15 | FEED | MT5 terminal + tick bridge in Docker | Sonnet | verifying | build committed on `nightmareinc/b15-mt5-docker` (74 unit, 11 Python tests); base image pull truncated in the sandbox; verification 2026-09-17 FAILED on a clean volume: the entrypoint raced the base image's Python install, bridge.py crashed, the container halted mid-install and restart-looped (10 restarts) with a poisoned volume; gates lint/unit/pytest green; second-pass ticket `docs/tickets/b15-harden-2.md` | M3: dispatch harden-2 when a seat is free |
