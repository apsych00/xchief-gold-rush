...
-- kiosks, coupons, dev_otps, otp_codes, devices, tournaments, tournament_scores and
-- badge_tiers deliberately have NO client policy: service-role tables (devices per ticket B5
-- decision 5 - "nothing identifies a device to other players"; a player's own device_id rides
-- on get_me() instead). The public top-10 leaderboard, the tournament header and the badge
-- legend are exposed through the SECURITY DEFINER functions public.leaderboard(),
-- public.current_tournament() and public.badge_legend() below, not a view: Supabase's linter
-- flags SECURITY DEFINER views, and a function keeps the same access model (anonymous visitors
-- can read the board, the tournament dates/prize and the badge legend, nothing else about
...
revoke insert, update, delete, truncate, references, trigger
  on public.players, public.rounds, public.tasks, public.task_claims, public.kiosks, public.coupons,
     public.tournaments, public.tournament_scores, public.badge_tiers
  from anon, authenticated;

-- kiosks, coupons, tournaments, tournament_scores and badge_tiers: not even readable by
-- clients. Service role only - the tournament header, the board and the badge legend reach the
-- client through the SECURITY DEFINER functions below, never a direct table read.
revoke select on public.kiosks, public.coupons, public.tournaments, public.tournament_scores, public.badge_tiers
  from anon, authenticated;
