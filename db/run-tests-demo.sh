#!/usr/bin/env bash
# Demo copy of db/run-tests.sh: identical, except it binds host port 55447 and names the
# kept container goldrush-demo-keep, so the showcase and red-team runs (demo/showcase.mjs,
# demo/redteam.mjs, docs/showcase.md) get their own database without disturbing a
# goldrush-box-keep container someone else is using on 55432.
#
# Apply db/schema.sql (the whole schema as one file) plus db/seed.sql on a throwaway STOCK
# postgres:16 container (not the supabase image, not the Supabase dev project), then run every
# pgTAP suite in db/tests/ with pg_prove.
#
#   bash db/run-tests-demo.sh --keep
#       leaves the container running on host port 55447 (user postgres, password test, db
